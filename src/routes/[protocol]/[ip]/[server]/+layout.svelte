	...
	<Navbar color="secondary-subtle" expand="lg" container="md" class="mb-2! border-t-2 border-(--bs-light-border-subtle)">
		...
		<Collapse {isOpen} navbar expand="md" on:update={handleUpdate}>
			<Nav class="ms-auto items-center" navbar>
				...
				<NavItem
					><NavLink href={basePath + "/settings"}>Settings</NavLink></NavItem
				>
				{#if con.state.servers[server].software !== "Vanilla"}
					<NavItem
						><NavLink href={basePath + "/mods"}
							>{con.state.servers[server].software === "Paper"
								? "Plugins"
								: "Mods"}</NavLink
						></NavItem
					>
				{/if}
				<NavItem
					><NavLink href={basePath + "/datapacks"}>Datapacks</NavLink></NavItem
				>
